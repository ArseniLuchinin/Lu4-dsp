...
cudaError_t launchDecimatorKernel(const cuComplex *in, cuComplex *out,
                                  size_t outCount, size_t phase, size_t sps,
...
bool runDecimatorCuda(const cuComplex *in, size_t inputSize, cuComplex *out,
                      int samplesPerSymbol, size_t currentPhase,
                      std::string *error) {
  if (inputSize == 0) {
    return true;
  }
  ...
  const int blocks =
      static_cast<int>((outCount + static_cast<size_t>(threads) - 1) /
                       static_cast<size_t>(threads));
  const auto launchErr = launchDecimatorKernel(in, out, outCount, currentPhase,
                                               sps, blocks, threads);
  if (launchErr != cudaSuccess) {
    if (error) {
      *error = std::string("Decimator::run failed: kernel launch failed: ") +
...
IModule *createModule() { return new Decimator(); }
...
bool Decimator::init() {
  if (m_samplesPerSymbol <= 0) {
    ERROR << "Decimator::init failed: samples per symbol must be > 0."
          << std::endl;
    return false;
  }
  ...
  if (m_offset < 0 || m_offset >= m_samplesPerSymbol) {
    ERROR
        << "Decimator::init failed: offset must be in [0, samples per symbol)."
        << std::endl;
    return false;
  }
  ...
  // Warm-up kernel/module path to avoid first-run latency spike on the first
  // real block.
  ...
  if (!warmupIn || !warmupOut || !warmupIn->isValid() ||
      !warmupOut->isValid()) {
    ERROR << "Decimator::init failed: unable to allocate warm-up buffers."
          << std::endl;
    return false;
  }
  ...
  if (!warmupIn->isValid()) {
    ERROR << "Decimator::init failed: unable to upload warm-up sample."
          << std::endl;
    return false;
  }

  std::string warmupError;
  if (!runDecimatorCuda(warmupIn->getDeviceData(), 1,
                        warmupOut->getDeviceData(), m_samplesPerSymbol, 0,
                        &warmupError)) {
  ...
  if (warmupSyncErr != cudaSuccess) {
    ERROR << "Decimator::init warm-up sync failed: "
          << cudaGetErrorString(warmupSyncErr) << std::endl;
    return false;
  }

  const auto tWarmupEnd = std::chrono::steady_clock::now();
  const auto warmupTotalUs =
      std::chrono::duration_cast<std::chrono::microseconds>(tWarmupEnd -
                                                            tWarmupStart)
          .count();
  DEBUG << "Decimator::init warm-up elapsed_total_us=" << warmupTotalUs
        << std::endl;
  return true;
}
...
bool Decimator::setData(std::shared_ptr<IData> data) {
  ...
  if (!m_inData) {
    ERROR << "Decimator::setData failed: input must be GpuComplexFloatSignal."
          << std::endl;
    return false;
  }
...
bool Decimator::run() {
  ...
  if (inputSize == 0) {
    ...
    if (!empty || !empty->isValid() || !empty->setLogicalSize(0)) {
      ERROR << "Decimator::run failed: unable to allocate empty output."
            << std::endl;
      return false;
    }
    m_outData = empty;
  ...
  const size_t outCount = (m_currentPhase < inputSize)
                              ? (1 + ((inputSize - 1 - m_currentPhase) / sps))
                              : 0;
  ...
  if (!m_outData) {
    ...
    if (!m_outData || !m_outData->isValid()) {
      ERROR << "Decimator::run failed: unable to allocate output GPU buffer."
            << std::endl;
      return false;
    }
  } else if (m_outData->availableSize() < requiredCapacity) {
    if (!m_outData->reserve(requiredCapacity) || !m_outData->isValid()) {
      ERROR << "Decimator::run failed: unable to grow output GPU buffer."
            << std::endl;
      return false;
    }
  }
  if (!m_outData->setLogicalSize(outCount)) {
    ERROR << "Decimator::run failed: unable to set output logical size."
          << std::endl;
    return false;
  }

  std::string error;
  if (!runDecimatorCuda(m_inData->getDeviceData(), inputSize,
                        m_outData->getDeviceData(), m_samplesPerSymbol,
                        m_currentPhase, &error)) {
...
void Decimator::setParam(const std::string &paramName, const std::any &value) {
  if (paramName == "samples per symbol") {
    ...
    if (!anyToInt(value, &parsed)) {
      ERROR << "Decimator::setParam failed: invalid samples per symbol type."
            << std::endl;
      return;
    }
...
std::shared_ptr<IData> Decimator::getData() { return m_outData; }
