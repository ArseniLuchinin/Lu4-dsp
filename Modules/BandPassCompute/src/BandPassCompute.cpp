...
IModule *createModule() { return new BandPassCompute(); }

BandPassCompute::BandPassCompute()
    : IModule(
          {"BandPassCompute", "libBandPassCompute-module.so", "module.json"}),
      m_sampleRate(0.0), m_filterOrder(0), m_lowCutoff(0.0), m_highCutoff(0.0) {
}
...
bool BandPassCompute::init() {
  if (m_sampleRate <= 0.0) {
    ERROR << "BandPassCompute::init failed: sample rate must be > 0."
          << std::endl;
    return false;
  }

  if (m_filterOrder <= 0 || (m_filterOrder % 2) == 0) {
    ERROR << "BandPassCompute::init failed: filter order must be positive odd "
             "number."
          << std::endl;
    return false;
  }
  ...
  if (!isBasicCutoffRangeValid(m_lowCutoff, m_highCutoff)) {
    ERROR << "BandPassCompute::init failed: cutoff range is invalid."
          << std::endl;
    return false;
  }
  if (!isNyquistBoundValid(m_sampleRate, m_highCutoff)) {
    ERROR << "BandPassCompute::init failed: high cutoff must be less than "
             "Nyquist frequency."
          << std::endl;
    return false;
  }
  ...
  for (int i = 0; i < m_filterOrder; ++i) {
    ...
    if (k == 0)
      ...
      val = (sin(2.0 * M_PI * f2 * k) - sin(2.0 * M_PI * f1 * k)) / (M_PI * k);
...
bool BandPassCompute::run() {
  if (!m_data) {
    ERROR << "BandPassCompute::run failed: coefficients were not initialized."
          << std::endl;
    return false;
  }
...
void BandPassCompute::setParam(const std::string &paramName,
                               const std::any &value) {
  if (paramName == "sample rate") {
    m_sampleRate = static_cast<double>(std::any_cast<int64_t>(value));
    DEBUG << "BandPassCompute sample rate set to: " << m_sampleRate
          << std::endl;
    return;
  }
  ...
  if (paramName == "filter order") {
    ...
    DEBUG << "BandPassCompute filter order set to: " << m_filterOrder
          << std::endl;
    return;
  }
  ...
  if (paramName == "block size") {
    ...
    DEBUG << "BandPassCompute::setParam: 'block size' is accepted for "
  ...
  if (paramName == "high cutoff") {
    ...
    DEBUG << "BandPassCompute high cutoff set to: " << m_highCutoff
          << std::endl;
    return;
  }

  ERROR << "BandPassCompute::setParam unknown parameter: " << paramName
        << std::endl;
}
