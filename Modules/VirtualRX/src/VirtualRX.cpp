...
IModule *createModule() { return new VirtualRX(); }
...
bool VirtualRX::setData(std::shared_ptr<IData> data) {
  (void)data;
  ERROR << "VirtualRX::setData is not supported for source module."
        << std::endl;
  return false;
}

std::shared_ptr<IData> VirtualRX::getData() { return m_data; }
