...
double rrcTapValue(double x, double rolloff)
{
    ...
    if (rolloff > 0.0) {
        ...
        if (std::abs(singularity - 1.0) < 1.0e-8) {
            ...
            return (rolloff / std::sqrt(2.0)) *
                (((1.0 + (2.0 / pi)) * std::sin(a)) +
    ...
    const double denominator =
        pi * x * (1.0 - std::pow(4.0 * rolloff * x, 2.0));
...
bool RRCCompute::init() {
    ...
    if (m_rolloff < 0.0 || m_rolloff > 1.0) {
        ERROR << "RRCCompute::init failed: rolloff must be inside [0, 1]." << std::endl;
        return false;
    }
    ...
    if (!m_samplesPerSymbolExplicit) {
        ...
        if (std::abs(ratio - rounded) > 1.0e-9) {
            ERROR << "RRCCompute::init failed: sample rate must be divisible by symbol rate when samples per symbol is not set." << std::endl;
            return false;
        }
    ...
    if (m_samplesPerSymbol <= 1) {
        ERROR << "RRCCompute::init failed: samples per symbol must be > 1." << std::endl;
        return false;
    }

    const int tapsCount = (m_spanSymbols * m_samplesPerSymbol) + 1;
    if (tapsCount <= 1 || (tapsCount % 2) == 0) {
        ERROR << "RRCCompute::init failed: taps count must be positive odd number." << std::endl;
        return false;
    }
    ...
    for (int i = 0; i < tapsCount; ++i) {
        ...
        const double x = static_cast<double>(n) / static_cast<double>(m_samplesPerSymbol);
        const double value = rrcTapValue(x, m_rolloff);
        if (!std::isfinite(value)) {
            ERROR << "RRCCompute::init failed: non-finite tap value generated." << std::endl;
            return false;
        }
    ...
    if (m_normalizeGain) {
        ...
        if (std::abs(sum) <= std::numeric_limits<double>::epsilon()) {
            ERROR << "RRCCompute::init failed: taps sum is zero, normalization is impossible." << std::endl;
            return false;
        }
...
bool RRCCompute::run() {
    if (!m_data) {
        ERROR << "RRCCompute::run failed: coefficients were not initialized." << std::endl;
        return false;
    }
