...
std::vector<float>
downloadGpuFloats(const std::shared_ptr<GpuFloatSignal> &data) {
...
std::vector<float> firFilter(const std::vector<float> &input, const float *taps,
                             size_t tapsCount) {
  ...
  for (size_t n = 0; n < input.size(); ++n) {
    ...
    for (size_t k = 0; k < tapsCount; ++k) {
      ...
      if (idx >= 0 && idx < static_cast<int>(input.size())) {
        acc += static_cast<double>(input[static_cast<size_t>(idx)]) *
               static_cast<double>(taps[k]);
      }
    }
