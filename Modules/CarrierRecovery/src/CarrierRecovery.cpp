...
extern cudaError_t carrierRecoveryQueryReduceStorageBytes(const cuComplex *in,
                                                          size_t n, int order,
                                                          size_t *storageBytes);
extern cudaError_t carrierRecoveryRunReduce(const cuComplex *in, size_t n,
                                            int order, void *tempStorage,
                                            size_t tempStorageBytes,
                                            double2 *outSum);
extern void launchCarrierRecoveryRotateKernel(const cuComplex *in,
                                              cuComplex *out, size_t n, float c,
                                              float s, int blocks, int threads);

IModule *createModule() { return new CarrierRecovery(); }

CarrierRecovery::CarrierRecovery()
    : IModule(
          {"CarrierRecovery", "libCarrierRecovery-module.so", "module.json"}) {}
...
bool CarrierRecovery::init() {
  ...
  if (!m_devSum) {
    const auto allocErr =
        cudaMalloc(reinterpret_cast<void **>(&m_devSum), sizeof(double2));
    if (allocErr != cudaSuccess) {
      ERROR << "CarrierRecovery::init failed: unable to allocate reduce output "
               "buffer: "
...
bool CarrierRecovery::setData(std::shared_ptr<IData> data) {
  ...
  if (!m_inData) {
    ERROR << "CarrierRecovery::setData failed: input must be "
             "GpuComplexFloatSignal."
          << std::endl;
    return false;
  }
  if (!m_inData->isValid()) {
    ERROR << "CarrierRecovery::setData failed: input signal is invalid."
          << std::endl;
    return false;
  }
...
bool CarrierRecovery::run() {
  ...
  auto out =
      std::make_shared<GpuComplexFloatSignal>(std::max<size_t>(size_t(1), n));
  if (!out || !out->isValid()) {
    ERROR << "CarrierRecovery::run failed: output allocation failed."
          << std::endl;
    return false;
  }
  if (!out->setLogicalSize(n)) {
    ERROR << "CarrierRecovery::run failed: unable to set output logical size."
          << std::endl;
    return false;
  }
  ...
  if (n > 0) {
    ...
    const auto queryErr = carrierRecoveryQueryReduceStorageBytes(
        m_inData->getDeviceData(), n, m_order, &requiredStorageBytes);
    if (queryErr != cudaSuccess) {
      ERROR
          << "CarrierRecovery::run failed: unable to query CUB reduce storage: "
    ...
    if (requiredStorageBytes > m_reduceTempStorageBytes) {
      ...
      if (allocTempErr != cudaSuccess) {
        ERROR << "CarrierRecovery::run failed: unable to allocate CUB temp "
                 "storage: "
    ...
    const auto reduceErr = carrierRecoveryRunReduce(
        m_inData->getDeviceData(), n, m_order, m_reduceTempStorage,
        m_reduceTempStorageBytes, m_devSum);
    ...
    const auto copyErr =
        cudaMemcpy(&hostSum, m_devSum, sizeof(double2), cudaMemcpyDeviceToHost);
    ...
    if (hostSum.x != 0.0 || hostSum.y != 0.0) {
      phase = std::atan2f(static_cast<float>(hostSum.y),
                          static_cast<float>(hostSum.x)) /
              static_cast<float>(m_order);

      if (m_order == 4) {
        phase -= static_cast<float>(M_PI_4);
      }

      const float wrapRange =
          static_cast<float>(M_PI) / static_cast<float>(m_order);
    ...
    launchCarrierRecoveryRotateKernel(m_inData->getDeviceData(),
                                      out->getDeviceData(), n, c, s, blocks,
                                      threads);
...
void CarrierRecovery::setParam(const std::string &paramName,
                               const std::any &value) {
  if (paramName == "order") {
    if (value.type() == typeid(int64_t)) {
      ...
      ERROR << "CarrierRecovery::setParam failed: invalid order type."
            << std::endl;
    }
    return;
  }

  ERROR << "CarrierRecovery::setParam unknown parameter: " << paramName
        << std::endl;
}

std::shared_ptr<IData> CarrierRecovery::getData() { return m_outData; }
