...
std::shared_ptr<GpuComplexFloatSignal>
makePhaseShiftedBpskSymbols(const std::vector<uint8_t> &bytes,
                            const float phase) {
  const std::vector<uint8_t> bits = bytesToMsbBits(bytes);
  const float c = std::cos(phase);
  const float s = std::sin(phase);
...
float phaseOf(const cuComplex &z) { return std::atan2f(z.y, z.x); }
...
TEST(CarrierRecoveryModuleTest, EstimatesPhaseCorrectly_QPSKOrder4) {
  ...
  for (const auto &z : hostOut) {
    const float residualPhase =
        std::remainder(phaseOf(z), static_cast<float>(M_PI_2));
    const float normalizedResidual =
        std::abs(residualPhase) - static_cast<float>(M_PI_4);
    EXPECT_NEAR(std::abs(normalizedResidual), 0.0f, 1.0e-3f)
        << "residual phase too large";
  }
}
...
TEST(CarrierRecoveryModuleTest, EstimatesPhaseCorrectly_BPSKOrder2) {
  ...
  for (const auto &z : hostOut) {
    const float residualPhase =
        std::remainder(phaseOf(z), static_cast<float>(M_PI));
...
TEST(CarrierRecoveryModuleTest, PhaseWrapNearBoundary) {
  ...
  const std::vector<float> phases = {quarterPi - 1.0e-4f, -quarterPi + 1.0e-4f};
  ...
  for (const float phi : phases) {
    ...
    for (const auto &z : hostOut) {
      ...
      const float normalizedResidual =
          std::abs(residualPhase) - static_cast<float>(M_PI_4);
      EXPECT_NEAR(std::abs(normalizedResidual), 0.0f, 2.0e-3f)
          << "phase wrap failed at phi=" << phi;
    }
  }
}
...
TEST(CarrierRecoveryModuleTest, AmplitudeImbalanceStillRecovers) {
  ...
  for (const auto &z : hostOut) {
    ...
    const float normalizedResidual =
        std::abs(residualPhase) - static_cast<float>(M_PI_4);
    EXPECT_NEAR(std::abs(normalizedResidual), 0.0f, 3.0e-3f)
        << "amplitude imbalance recovery failed";
  }
}
...
TEST(CarrierRecoveryModuleTest, StatelessAcrossBlocks) {
  ...
  for (const auto &z : hostOut1) {
    ...
    const float normalizedResidual =
        std::abs(residualPhase) - static_cast<float>(M_PI_4);
    EXPECT_NEAR(std::abs(normalizedResidual), 0.0f, 1.0e-3f)
        << "block 1 residual phase too large";
  }
  ...
  for (const auto &z : hostOut2) {
    ...
    const float normalizedResidual =
        std::abs(residualPhase) - static_cast<float>(M_PI_4);
    EXPECT_NEAR(std::abs(normalizedResidual), 0.0f, 1.0e-3f)
        << "block 2 residual phase too large";
  }
}
