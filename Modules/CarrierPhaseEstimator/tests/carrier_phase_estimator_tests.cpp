...
float downloadSingleGpuFloat(const std::shared_ptr<GpuFloatSignal> &data,
                             bool *ok = nullptr) {
  if (ok) {
    *ok = false;
  }
  if (!data || !data->isValid() || data->size() == 0 ||
      !data->getDeviceData()) {
    return 0.0f;
  }

  float value = 0.0f;
  const auto err = cudaMemcpy(&value, data->getDeviceData(), sizeof(float),
...
std::shared_ptr<GpuComplexFloatSignal>
scaleSymbols(const std::shared_ptr<GpuComplexFloatSignal> &in,
  ...
  for (size_t i = 0; i < n; ++i) {
    const float amp =
        amplitudes.empty() ? 1.0f : amplitudes[i % amplitudes.size()];
    host[i].x *= amp;
    host[i].y *= amp;
  }
...
TEST(CarrierPhaseEstimatorModuleTest, UsesFourthPower) {
  ...
  for (size_t i = 0; i < 64; ++i) {
    ...
    symbols.emplace_back(std::cos(a) * amp * std::sqrt(2.0f),
                         std::sin(a) * amp * std::sqrt(2.0f));
  }
  ...
  auto phaseData = std::dynamic_pointer_cast<GpuFloatSignal>(
      tx.waitRxData("phase_test_tag_1"));
  ...
  ASSERT_TRUE(phaseOk);
  const float wrappedErr =
      std::remainder(measured - phi, static_cast<float>(M_PI_2));
...
TEST(CarrierPhaseEstimatorModuleTest, EmptyInputProducesZeroPhase) {
  ...
  VirtualTransmitter tx;
  auto phaseData = std::dynamic_pointer_cast<GpuFloatSignal>(
      tx.waitRxData("phase_test_tag_empty"));
  ASSERT_NE(phaseData, nullptr);
  ASSERT_EQ(phaseData->size(), size_t(1));
...
TEST(CarrierPhaseEstimatorModuleTest, AmplitudeImbalanceStillEstimatesPhase) {
  ...
  const std::vector<uint8_t> bytes = {'A', 'm', 'p', 'l', 'i',
                                      't', 'u', 'd', 'e', '\n'};
  ...
  VirtualTransmitter tx;
  auto phaseData = std::dynamic_pointer_cast<GpuFloatSignal>(
      tx.waitRxData("phase_test_tag_amp_imbalance"));
  ASSERT_NE(phaseData, nullptr);
  ASSERT_EQ(phaseData->size(), size_t(1));
...
TEST(CarrierPhaseEstimatorModuleTest, PhaseWrapAroundNearPiOver4) {
  ...
  const std::vector<float> phases = {static_cast<float>(M_PI_4) - 1.0e-4f,
  ...
  for (const float phi : phases) {
    ...
    auto phaseData = std::dynamic_pointer_cast<GpuFloatSignal>(
        tx.waitRxData("phase_test_tag_wrap"));
    ASSERT_NE(phaseData, nullptr);
    ASSERT_EQ(phaseData->size(), size_t(1));
