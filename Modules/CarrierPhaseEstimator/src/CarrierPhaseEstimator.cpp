...
bool CarrierPhaseEstimator::setData(std::shared_ptr<IData> data) {
    ...
    if (!m_inData) {
        ERROR << "CarrierPhaseEstimator::setData failed: input must be GpuComplexFloatSignal." << std::endl;
        return false;
    }
    if (!m_inData->isValid()) {
...
bool CarrierPhaseEstimator::run() {
    ...
    if (n > 0) {
        if (!m_reduceOut) {
            const auto allocOutErr = cudaMalloc(reinterpret_cast<void**>(&m_reduceOut), sizeof(double2));
            if (allocOutErr != cudaSuccess) {
                ERROR << "CarrierPhaseEstimator::run failed: unable to allocate reduce output buffer: "
        ...
        if (queryErr != cudaSuccess) {
            ERROR << "CarrierPhaseEstimator::run failed: unable to query CUB reduce storage: "
        ...
        if (requiredStorageBytes > m_reduceTempStorageBytes) {
            ...
            if (allocTempErr != cudaSuccess) {
                ERROR << "CarrierPhaseEstimator::run failed: unable to allocate CUB temp storage: "
        ...
        const auto reduceErr = carrierPhaseRunReduce(
            ...
            static_cast<double2*>(m_reduceOut)
        );
        ...
        const auto copySumErr = cudaMemcpy(
            &sum,
            m_reduceOut,
            sizeof(double2),
        ...
        if (copySumErr != cudaSuccess) {
            ERROR << "CarrierPhaseEstimator::run failed: unable to copy reduce result to host: "
    ...
    if (n == 0 || (sum.x == 0.0 && sum.y == 0.0)) {
        ...
        float phase = static_cast<float>(std::atan2(sum.y, sum.x) * 0.25) - quarterPi;
    ...
    if (!phaseData || !phaseData->isValid()) {
        ERROR << "CarrierPhaseEstimator::run failed: unable to allocate phase GPU buffer." << std::endl;
        return false;
    }
    phaseData->setDataFromHost(&m_estimatedPhase, 1);
    if (!phaseData->isValid()) {
        ERROR << "CarrierPhaseEstimator::run failed: unable to upload phase to GPU." << std::endl;
        return false;
    }
