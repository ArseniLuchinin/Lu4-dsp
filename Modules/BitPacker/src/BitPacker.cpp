...
cudaError_t launchBitPackerPackKernel(
    const uint8_t *pendingBits, size_t pendingCount, const uint8_t *inBits,
    size_t inCount, size_t drop, size_t fullBytes, size_t remBits, bool hasTail,
    uint8_t *outBytes, size_t outBytesCount, int blocks, int threads);

cudaError_t
launchBitPackerPendingKernel(const uint8_t *pendingBits, size_t pendingCount,
                             const uint8_t *inBits, size_t inCount, size_t drop,
                             size_t effectiveBits, uint8_t *nextPendingBits,
                             size_t nextPendingCount, int blocks, int threads);
...
IModule *createModule() { return new BitPacker(); }
...
bool BitPacker::init() {
  if (m_bitOrder != "msb-first") {
    ERROR << "BitPacker::init failed: only 'msb-first' is supported in MVP."
          << std::endl;
    return false;
  }
  ...
  if (!m_pendingBitsData || !m_pendingBitsData->isValid()) {
    ERROR
        << "BitPacker::init failed: unable to allocate pending bits GPU buffer."
        << std::endl;
    return false;
  }
  if (!m_pendingBitsData->setLogicalSize(0)) {
    ERROR << "BitPacker::init failed: unable to set pending bits logical size."
          << std::endl;
    return false;
  }
...
bool BitPacker::setData(std::shared_ptr<IData> data) {
  ...
  if (!m_inData) {
    ERROR << "BitPacker::setData failed: input must be GpuByteSignal with bits."
          << std::endl;
    return false;
  }
...
bool BitPacker::run() {
  ...
  if (!m_pendingBitsData || !m_pendingBitsData->isValid()) {
    ERROR << "BitPacker::run failed: pending bits buffer is not initialized."
          << std::endl;
    return false;
  }
  ...
  auto outData = std::make_shared<GpuByteSignal>(
      std::max<size_t>(size_t(1), outBytesCount));
  ...
  if (!outData->setLogicalSize(outBytesCount)) {
    ERROR << "BitPacker::run failed: unable to set output logical size."
          << std::endl;
    return false;
  }
  ...
  if (outBytesCount > 0) {
    ...
    const int blocks =
        static_cast<int>((outBytesCount + static_cast<size_t>(threads) - 1) /
                         static_cast<size_t>(threads));
    const auto packErr = launchBitPackerPackKernel(
        m_pendingBitsData->getDeviceData(), m_pendingBitsCount,
        m_inData->getDeviceData(), inCount, drop, fullBytes, remBits, hasTail,
        outData->getDeviceData(), outBytesCount, blocks, threads);
    if (packErr != cudaSuccess) {
      ERROR << "BitPacker::run failed: pack kernel launch failed: "
            << cudaGetErrorString(packErr) << std::endl;
      return false;
    }
  }
  ...
  if (nextPendingCount > 0) {
    const int threads = 256;
    const int blocks =
        static_cast<int>((nextPendingCount + static_cast<size_t>(threads) - 1) /
                         static_cast<size_t>(threads));
    const auto pendingErr = launchBitPackerPendingKernel(
        m_pendingBitsData->getDeviceData(), m_pendingBitsCount,
        m_inData->getDeviceData(), inCount, drop, effectiveBits,
        m_pendingBitsData->getDeviceData(), nextPendingCount, blocks, threads);
    if (pendingErr != cudaSuccess) {
      ERROR << "BitPacker::run failed: pending kernel launch failed: "
            << cudaGetErrorString(pendingErr) << std::endl;
      return false;
    }
  }
  ...
  if (syncErr != cudaSuccess) {
    ERROR << "BitPacker::run failed: kernel execution failed: "
  ...
  if (!m_pendingBitsData->setLogicalSize(m_pendingBitsCount)) {
    ERROR << "BitPacker::run failed: unable to set pending bits logical size."
          << std::endl;
    return false;
  }
...
void BitPacker::setParam(const std::string &paramName, const std::any &value) {
  ...
  if (paramName == "discard leading bits") {
    ...
    if (!anyToSize(value, &parsed)) {
      ERROR << "BitPacker::setParam failed: invalid discard leading bits "
               "type/value."
            << std::endl;
      return;
    }
...
std::shared_ptr<IData> BitPacker::getData() { return m_outData; }
