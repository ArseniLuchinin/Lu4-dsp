...
IModule *createModule() { return new FFT(); }
...
bool FFT::init() {
  ...
  if (m_hopSize > m_fftSize) {
    ERROR << "FFT::init: hop size must be less than or equal to fft size."
          << std::endl;
    return false;
  }
...
bool FFT::initPlan(int batchCount) {
  ...
  const auto planStatus =
      cufftMakePlanMany(m_plan, 1, n, nullptr, istride, idist, nullptr, ostride,
                        odist, m_impl->planType(), batchCount, &workSize);
...
bool FFT::run() {
  ...
  if (!m_impl->execute(m_plan, m_fftSize, m_hopSize, m_overlapSize,
                       m_isFirstRun, m_batchCount)) {
    ERROR << "FFT::run: implementation execute failed. " << m_impl->lastError()
          << std::endl;
    cufftDestroy(m_plan);
    m_plan = 0;
    return false;
...
bool FFT::setData(std::shared_ptr<IData> data) {
  if (std::dynamic_pointer_cast<GpuFloatSignal>(data)) {
    if (!m_impl) {
      ...
      ERROR << "FFT::setData: implementation type changed unexpectedly."
            << std::endl;
      return false;
    }
    ...
    ERROR << "FFT::setData: input IData is not supported signal type."
          << std::endl;
    return false;
  }
  ...
  if (m_hopSize == 0) {
    ERROR << "FFT::setData: init() was not called or hop size is invalid."
          << std::endl;
    return false;
  }
  ...
  if (batch <= 0) {
    ERROR << "FFT::setData: no FFT frames to process for current input."
          << std::endl;
    return false;
  }
