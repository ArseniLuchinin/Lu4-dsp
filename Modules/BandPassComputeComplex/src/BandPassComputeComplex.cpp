...
IModule* createModule() {
    return new BandPassComputeComplex();
}
...
bool BandPassComputeComplex::init() {
    if (m_sampleRate <= 0.0) {
        ERROR << "BandPassComputeComplex::init failed: sample rate must be > 0." << std::endl;
        return false;
    }

    if (m_filterOrder <= 0 || (m_filterOrder % 2) == 0) {
        ERROR << "BandPassComputeComplex::init failed: filter order must be positive odd number." << std::endl;
        return false;
    }
    ...
    if (m_sidebandExplicit && m_sideband == Sideband::Both) {
        if (m_lowCutoff < 0.0 || m_highCutoff <= m_lowCutoff) {
            ERROR << "BandPassComputeComplex::init failed: cutoff range is invalid for sideband='both'." << std::endl;
            return false;
        }
        if (m_highCutoff >= nyquist) {
            ERROR << "BandPassComputeComplex::init failed: high cutoff must be less than Nyquist frequency." << std::endl;
            return false;
        }
    ...
    if (m_sidebandExplicit) {
        if (m_lowCutoff < 0.0 || m_highCutoff <= m_lowCutoff) {
            ERROR << "BandPassComputeComplex::init failed: cutoff range is invalid for explicit sideband mode." << std::endl;
            return false;
        }
        if (m_highCutoff >= nyquist) {
            ERROR << "BandPassComputeComplex::init failed: high cutoff must be less than Nyquist frequency." << std::endl;
        ...
        if (m_highCutoff <= m_lowCutoff) {
            ERROR << "BandPassComputeComplex::init failed: signed cutoff range is invalid." << std::endl;
            return false;
        }
        if (m_lowCutoff <= -nyquist || m_highCutoff >= nyquist) {
            ERROR << "BandPassComputeComplex::init failed: signed cutoffs must be inside (-Nyquist, Nyquist)." << std::endl;
            return false;
        }
    ...
    for (int i = 0; i < m_filterOrder; ++i) {
        ...
        coeff[i] = make_cuComplex(
            static_cast<float>(env * std::cos(phase)),
...
bool BandPassComputeComplex::run() {
    if (!m_data) {
        ERROR << "BandPassComputeComplex::run failed: coefficients were not initialized." << std::endl;
        return false;
    }
...
void BandPassComputeComplex::setParam(const std::string& paramName, const std::any& value) {
    ...
    ERROR << "BandPassComputeComplex::setParam unknown parameter: " << paramName << std::endl;
}
