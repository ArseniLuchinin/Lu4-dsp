...
bool resolveFreqBins(size_t fftSize, size_t windowSize, size_t totalSize,
                     size_t *freqBins, bool *isRealSpectrum) {
  ...
  const bool divisibleReal =
      (totalSize >= realBins) && ((totalSize % realBins) == 0);
  const bool divisibleComplex =
      (totalSize >= complexBins) && ((totalSize % complexBins) == 0);
...
IModule *createModule() { return new SpectrogramPlot(); }

SpectrogramPlot::SpectrogramPlot()
    : IModule(
          {"SpectrogramPlot", "libSpectrogramPlot-module.so", "module.json"}) {}
...
bool SpectrogramPlot::init() { return true; }

int64_t i = 0;
bool SpectrogramPlot::run() {
  if (!m_data || !m_data->isValid()) {
    ERROR << "SpectrogramPlot::run: input data is null or invalid."
          << std::endl;
    return false;
  }
  ...
  if (!resolveFreqBins(m_fftSize, m_windowSize, totalSize, &m_freqBins,
                       &isRealSpectrum)) {
    ERROR
        << "SpectrogramPlot::run: failed to determine valid spectrogram width. "
        << "Expected window size " << ((m_fftSize / 2) + 1) << " (real) or "
        << m_fftSize << " (complex)." << std::endl;
    return false;
  }

  if (totalSize < m_freqBins) {
    ERROR << "SpectrogramPlot::run: input size (" << totalSize
          << ") is smaller than one spectrogram row (" << m_freqBins << ")."
          << std::endl;
    return false;
  }
  ...
  if ((totalSize % m_freqBins) != 0) {
    ERROR << "SpectrogramPlot::run: input size (" << totalSize
          << ") is not divisible by spectrogram row size (" << m_freqBins
          << ")." << std::endl;
    return false;
  }

  if (m_centeredSpectrum && isRealSpectrum) {
    ERROR << "SpectrogramPlot::run: centered spectrum is supported only for "
  ...
  if (m_hasDbRange && !(m_dbMax > m_dbMin)) {
    ERROR << "SpectrogramPlot::run: invalid dB range, 'db max' must be greater "
             "than 'db min'."
          << std::endl;
    return false;
  }
  ...
  if (m_hasFreqRange) {
    ...
    colOffset = static_cast<size_t>(std::max(0.0, std::min(1.0, startFrac)) *
                                    (m_freqBins - 1));
    const size_t endCol = static_cast<size_t>(
        std::max(0.0, std::min(1.0, endFrac)) * (m_freqBins - 1));

    if (endCol <= colOffset) {
      ERROR << "SpectrogramPlot::run: freq range [" << m_freqMin << ", "
            << m_freqMax << "] is outside the spectrum range [" << fullFreqMin
            << ", " << fullFreqMax << "]." << std::endl;
      return false;
    }
  ...
  if (renderCols == 0) {
    ERROR << "SpectrogramPlot::run: no columns to render in the specified freq "
             "range."
          << std::endl;
    return false;
  }
  ...
  if (m_hasDbRange) {
    ...
    const auto copyErr =
        cudaMemcpy(m_hostBuffer.data(), m_data->getDeviceData(),
                   totalSize * sizeof(float), cudaMemcpyDeviceToHost);
    if (copyErr != cudaSuccess) {
      ERROR << "SpectrogramPlot::run: cudaMemcpy failed: "
            << cudaGetErrorString(copyErr) << std::endl;
      return false;
    }

    const auto [minIt, maxIt] =
        std::minmax_element(m_hostBuffer.begin(), m_hostBuffer.end());
    minValue = *minIt;
    maxValue = *maxIt;
  }
  ...
  if (!m_gpuColorLut) {
    ...
    const auto allocErr =
        cudaMalloc(reinterpret_cast<void **>(&m_gpuColorLut), lutBytes);
    if (allocErr != cudaSuccess) {
      ERROR << "SpectrogramPlot::run: failed to allocate GPU LUT: "
            << cudaGetErrorString(allocErr) << std::endl;
      return false;
    }
    const auto copyLutErr = cudaMemcpy(m_gpuColorLut, m_colorLutHost.data(),
                                       lutBytes, cudaMemcpyHostToDevice);
  ...
  if (!m_gpuImageBuffer || m_gpuImageCapacity < imageBytes) {
    ...
    const auto allocErr =
        cudaMalloc(reinterpret_cast<void **>(&m_gpuImageBuffer), imageBytes);
    if (allocErr != cudaSuccess) {
  ...
  if (!renderSpectrogramImageCuda(
          m_data->getDeviceData(), m_gpuImageBuffer, rows, renderCols,
          colOffset, m_freqBins, minValue, maxValue, m_hasMaskBelowDb,
          static_cast<float>(m_maskBelowDb), m_gpuColorLut)) {
  ...
  const auto imageCopyErr = cudaMemcpy(image.data, m_gpuImageBuffer, imageBytes,
  ...
  const int canvasWidth =
      leftMargin + static_cast<int>(renderCols) + rightMargin;
  ...
  for (int t = 0; t <= freqTicks; ++t) {
    ...
    const int x =
        leftMargin + static_cast<int>(std::round(frac * (renderCols - 1)));
    const double freq = freqMin + (freqMax - freqMin) * frac;
    const std::string label =
        std::to_string(static_cast<int>(std::round(freq)));
    cv::putText(canvas, label, cv::Point(x - 10, topMargin - 8), font,
                fontScale, color, thickness);
    cv::line(canvas, cv::Point(x, topMargin - 5), cv::Point(x, topMargin),
             color, 1);
  }

  const int timeTicks = 5;
  const size_t hopSize = (m_hopSize > 0) ? m_hopSize : m_fftSize;
  const double secondsPerRow =
      (m_sampleRate > 0) ? (static_cast<double>(hopSize) / m_sampleRate) : 0.0;
  for (int t = 0; t <= timeTicks; ++t) {
    ...
    cv::putText(canvas, label, cv::Point(5, y + 4), font, fontScale, color,
                thickness);
    cv::line(canvas, cv::Point(leftMargin - 5, y), cv::Point(leftMargin, y),
             color, 1);
  }

  const std::string outPath = m_savePath.empty()
                                  ? "spectrogram" + std::to_string(i) + ".png"
                                  : m_savePath + std::to_string(i) + ".png";
  ...
  if (!cv::imwrite(outPath, canvas, writeParams)) {
    ERROR << "SpectrogramPlot::run: failed to save image to " << outPath
          << std::endl;
    return false;
  }
  ...
  if (m_isShow) {
    INFO << "SpectrogramPlot::run: show=true is set, but interactive preview "
            "is not implemented."
         << std::endl;
  }
...
void SpectrogramPlot::setParam(const std::string &paramName,
                               const std::any &value) {
...
bool SpectrogramPlot::setData(std::shared_ptr<IData> data) {
  ...
  if (!gpuData) {
    ERROR << "SpectrogramPlot::run: input data is not a valid GpuFloatSignal."
          << std::endl;
    return false;
  }
...
std::shared_ptr<IData> SpectrogramPlot::getData() { return m_transitData; }
