...
IModule *createModule() { return new FileSrc(); }
...
void FileSrc::setParam(const std::string &paramName, const std::any &value) {
  ...
  if (paramName == "max size") {
    if (const auto *v = std::any_cast<size_t>(&value)) {
      m_maxSize = *v;
    } else if (const auto *v64 = std::any_cast<int64_t>(&value)) {
      m_maxSize = static_cast<size_t>(*v64);
    } else {
...
bool FileSrc::setData(std::shared_ptr<IData> data) { return false; }

std::shared_ptr<IData> FileSrc::getData() { return m_data; }
...
bool FileSrc::readFile() {
  ...
  const size_t elementSize =
      (m_type == DataType::Float) ? sizeof(float) : sizeof(cuComplex);
  ...
  if (m_type == DataType::Float) {
    ...
    tryData = std::shared_ptr<GpuComplexFloatSignal>(
        new GpuComplexFloatSignal(elementCount));
  }

  uint8_t *srcPtr = m_mmapPtr + m_offset;
  INFO << "Readed size: " << elementCount << " from file: " << m_fileName
       << std::endl;
  if (m_type == DataType::Float) {
    std::dynamic_pointer_cast<GpuFloatSignal>(tryData)->setDataFromHost(
        reinterpret_cast<float *>(srcPtr), elementCount);
  } else {
    std::dynamic_pointer_cast<GpuComplexFloatSignal>(tryData)->setDataFromHost(
        reinterpret_cast<cuComplex *>(srcPtr), elementCount);
  }
