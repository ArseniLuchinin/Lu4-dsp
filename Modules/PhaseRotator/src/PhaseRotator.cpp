...
IModule *createModule() { return new PhaseRotator(); }
...
bool PhaseRotator::setData(std::shared_ptr<IData> data) {
  ...
  if (!m_inData) {
    ERROR
        << "PhaseRotator::setData failed: input must be GpuComplexFloatSignal."
        << std::endl;
    return false;
  }

  if (!m_inData->isValid()) {
    ERROR << "PhaseRotator::setData failed: input signal is invalid."
          << std::endl;
    return false;
  }
...
bool PhaseRotator::run() {
  ...
  if (!phaseData || phaseData->size() == 0 || !phaseData->getDeviceData()) {
    ERROR << "PhaseRotator::run failed: phase data is missing or invalid."
          << std::endl;
    return false;
  }

  float phase = 0.0f;
  const auto phaseCopyErr = cudaMemcpy(&phase, phaseData->getDeviceData(),
  ...
  if (!out->setLogicalSize(n)) {
    ERROR << "PhaseRotator::run failed: unable to set output logical size."
          << std::endl;
    return false;
  }

  extern cudaError_t launchPhaseRotatorKernel(const cuComplex *in,
  ...
  if (n > 0) {
    ...
    const auto launchErr = launchPhaseRotatorKernel(m_inData->getDeviceData(),
                                                    out->getDeviceData(), n, c,
                                                    s, blocks, threads);
    if (launchErr != cudaSuccess) {
      ERROR << "PhaseRotator::run failed: kernel launch failed: "
    ...
    if (syncErr != cudaSuccess) {
      ERROR << "PhaseRotator::run failed: kernel execution failed: "
...
void PhaseRotator::setParam(const std::string &paramName,
                            const std::any &value) {
  ...
  ERROR << "PhaseRotator::setParam unknown parameter: " << paramName
        << std::endl;
}

std::shared_ptr<IData> PhaseRotator::getData() { return m_outData; }
