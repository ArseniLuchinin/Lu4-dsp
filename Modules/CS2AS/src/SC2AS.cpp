...
namespace {
__global__ void complexToAmplitudeKernel(const cuComplex *in, float *out,
                                         size_t n, float powerNorm) {
...
IModule *createModule() { return new CS2AS(); }

bool CS2AS::init() { return true; }

bool CS2AS::run() {
  if (!m_inData || !m_inData->isValid()) {
    ERROR << "CS2AS::run: input complex spectrum is null or invalid."
          << std::endl;
    return false;
  }
  ...
  if (launchErr != cudaSuccess) {
    ERROR << "CS2AS::run: kernel launch failed: "
  ...
  if (syncErr != cudaSuccess) {
    ERROR << "CS2AS::run: kernel execution failed: "
...
std::shared_ptr<IData> CS2AS::getData() { return m_outData; }
