...
IModule *createModule() { return new FIRFilter(); }

FIRFilter::FIRFilter()
    : IModule({"FIR-filter", "libFIR-filter-module.so", "module.json"}) {}
FIRFilter::~FIRFilter() {
  if (m_energyBuffer) {
...
bool FIRFilter::init() {
  if (m_M <= 0 || (m_M % 2) == 0) {
    ERROR << "FIRFilter::init failed: filter order must be positive odd number."
          << std::endl;
    return false;
  }

  if (!m_taps->isValid()) {
    ERROR << "FIRFilter::init failed: coefficients data is invalid."
          << std::endl;
    return false;
  }
  ...
  if (tapsSize != static_cast<size_t>(m_M)) {
    ERROR
        << "FIRFilter::init failed: coefficients size must match filter order."
        << std::endl;
    return false;
  }
  ...
  if (m_coefficientsTypeMode == CoefficientsTypeMode::Real &&
    ...
    ERROR << "FIRFilter::init failed: coefficients type mode is 'real', but "
             "complex taps were provided."
          << std::endl;
    return false;
  }
  ...
  if (m_coefficientsTypeMode == CoefficientsTypeMode::Complex &&
    ...
    ERROR << "FIRFilter::init failed: coefficients type mode is 'complex', but "
             "real taps were provided."
          << std::endl;
    return false;
  }
  ...
  INFO << "FIRFilter::init: taps initialized, order=" << m_M << ", tap type="
...
bool FIRFilter::setData(std::shared_ptr<IData> data) {
  if (m_M <= 0) {
    ERROR << "FIRFilter::setData failed: filter order is not initialized."
          << std::endl;
    return false;
  }

  if (!m_taps) {
    ERROR << "FIRFilter::setData failed: taps are not initialized."
          << std::endl;
    return false;
  }
  ...
  auto validation = validateGpuInput(
      data, {GpuSampleType::Float32, GpuSampleType::ComplexFloat32},
      historySize, "FIRFilter::setData");
  if (!validation.ok) {
  ...
  if (m_taps->sampleType() == GpuSampleType::ComplexFloat32 &&
    ...
    ERROR << "FIRFilter::setData failed: complex taps require complex input "
             "signal."
          << std::endl;
    return false;
  }
  ...
  const bool needWorkRecreate =
      !m_workData || m_workData->sampleType() != m_gpuData->sampleType() ||
      m_workData->availableSize() < m_gpuData->size();

  if (needWorkRecreate) {
    m_workData = createLike(*m_gpuData, m_gpuData->size());
    if (!m_workData || !m_workData->isValid()) {
      ERROR << "FIRFilter::setData failed: unable to allocate work buffer."
            << std::endl;
      return false;
    }
  } else if (!m_workData->setLogicalSize(m_gpuData->size())) {
    ERROR << "FIRFilter::setData failed: unable to set work buffer size."
          << std::endl;
    return false;
  }

  auto historyValidation = ensureHistoryLike(*m_gpuData, historySize,
                                             m_historyData, m_nextHistoryData);
  if (!historyValidation.ok) {
    ERROR << "FIRFilter::setData failed: " << historyValidation.error
          << std::endl;
    return false;
  }
  ...
  const auto err = cudaMemcpy(nextHistoryBytes, inputBytes + offsetBytes,
                              totalBytes, cudaMemcpyDeviceToDevice);
  if (err != cudaSuccess) {
...
void FIRFilter::setParam(const std::string &paramName, const std::any &value) {
  ...
  if (paramName == "coefficients type") {
    ...
    {
      ERROR << "FIRFilter::setParam failed: unknown coefficients type '"
            << modeStr << "'." << std::endl;
    }
    return;
  }
  ...
  if (paramName == "taps") {
    if (const auto *taps = std::any_cast<std::shared_ptr<IData>>(&value)) {
      ...
      if (!m_taps) {
        ERROR
            << "FIRFilter::setParam failed: 'taps' must reference a GPU signal."
            << std::endl;
      }
      return;
    }

    ERROR << "FIRFilter::setParam failed: unsupported type for 'taps'."
          << std::endl;
    return;
  }
...
std::shared_ptr<IData> FIRFilter::getData() { return m_data; }
