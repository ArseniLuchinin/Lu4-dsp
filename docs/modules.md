...
| `getMetaData()` | Возврат пути к `module.json` — файлу метаданных модуля, который содержит имя, описание и типы параметров. |
...
**Фаза параметризации.** Сразу после создания ConveyorFactory вызывает `module->getMetaData()` для получения пути к `module.json`, создаёт `ModuleMethaDataReader` и определяет ожидаемые типы полей. Затем обходит все параметры из `pipeline.json` и для каждого сначала приводит JSON-значение к нужному C++-типу через `convertParamValue()`, а затем вызывает `fetchParam`. Этот метод — «умный мост» между JSON и C++: он распознаёт три вида значений и преобразует их в `std::any`. Если значение начинается с `$`, `fetchParam` подменяет его на переменную из `variables.toml`. Если со `@`, модуль регистрируется как получатель данных через `VirtualTransmitter` и блокируется до их поступления. Обычное значение передаётся в `setParam` как есть. Такой подход удобен, потому что один и тот же `pipeline.json` можно запускать с разными наборами параметров, просто подменяя `variables.toml` — например, для dev- и prod-окружений.
...
## Метаданные модуля (module.json)

Каждый модуль обязан содержать файл `module.json` в своей директории. Этот файл описывает параметры модуля и их типы, что позволяет `ConveyorFactory` строго контролировать приведение JSON-значений к C++-типам.
...
```json
{
  ...
  "fields": [
    { "name": "param_name", "type": "int", "label": "Описание параметра" },
    { "name": "factor", "type": "real", "label": "Коэффициент" },
    { "name": "mode", "type": "enum", "label": "Режим работы" },
    { "name": "enabled", "type": "bool", "label": "Включено" },
    { "name": "path", "type": "string", "label": "Путь к файлу" }
  ]
}
```

Типы полей и соответствующие C++-типы:

| Тип поля | C++-тип | Описание |
|---|---|---|
| `string` | `std::string` | Строковое значение |
| `enum` | `std::string` | Строковое значение из ограниченного набора |
| `int` | `int64_t` | Целое число |
| `real` | `double` | Вещественное число |
| `bool` | `bool` | Логическое значение |

Зачем это нужно. Раньше JSON-числа могли неправильно интерпретироваться на границе JSON → C++ (например, `int64` → `int32_t` или `float` → `int`). Теперь `ConveyorFactory` через `ModuleMethaDataReader` читает ожидаемый тип из `module.json` и явно приводит значение через `convertParamValue()`. Это исключает неявные truncation, rounding и undefined behavior при передаче параметров.
...
### FileWriter
- **Назначение:** Запись бинарных данных в файл
- **Параметры:** `file name` (string)
- **Логика:** Получает данные от предыдущего модуля и записывает их в бинарный файл. Удобен для сохранения промежуточных результатов конвейера.
