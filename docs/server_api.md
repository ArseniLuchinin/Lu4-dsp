...
|---|---|---|
| `SERVER_PORT` | Порт HTTP-сервера | `5000` |
...
8. STDOUT и STDERR вычислительного сервера перенаправляются в лог-файл `logs_YYYYMMDD_HHMMSS.txt`.
...
- **Нет endpoint'а статуса сессии:** после запуска клиент не может запросить состояние запущенного процесса или прочитать логи через API.
...
- **Файловые дескрипторы:** лог-файл открывается для записи и передаётся в `Popen`. При успешном запуске файл остаётся открытым у дочернего процесса; при ошибке запуска файл закрывается в обработчике исключения.
