...
Извлекает `"name"` — имя модуля. Вызывает `ModuleFactory::createModule(name)` для создания экземпляра. Если в JSON есть `"params"`, для каждого параметра вызывает `module->fetchParam(name, jsonToAny(value))`. Затем добавляет модуль в конвейер через `conveyor.addModule(module)`.

Здесь важен порядок: сначала создание, потом параметризация, потом добавление. Если `fetchParam` вернёт `false` (например, переменная не найдена или таймаут `@tag`), модуль не добавляется, а конвейер помечается как невалидный.
...
Эта конвертация необходима, потому что `setParam` принимает `std::any`, а JSON не имеет прямого отображения в C++-типы. Использование `std::any` позволяет модулю самому решать, какой тип ожидать, и делать `any_cast` с проверкой. Это даёт гибкость: один и тот же параметр может принимать `int` или `double`, и модуль сам разберётся.
