...
CpuComplexSignal::CpuComplexSignal(cuComplex *data, size_t size)
    : IData("CPU complex signal"), m_data(data), m_size(size) {}

std::shared_ptr<CpuComplexSignal>
CpuComplexSignal::fromGpu(std::shared_ptr<IData> iData) {
  auto gpuData = std::dynamic_pointer_cast<GpuComplexFloatSignal>(iData);
  if (gpuData == nullptr) {
    std::cerr << "CpuComplexSignal::fromGpu: Input IData is not a "
  ...
  if (gpuData->size() == 0) {
    std::cerr << "CpuComplexSignal::fromGpu: GpuComplexFloatSignal has size 0."
              << std::endl;
    return std::make_shared<CpuComplexSignal>();
  }
  ...
  cudaError_t err =
      cudaMemcpy(data, gpuData->getDeviceData(),
                 gpuData->size() * sizeof(cuComplex), cudaMemcpyDeviceToHost);
  if (err != cudaSuccess) {
    std::cerr << "CpuComplexSignal::fromGpu: cudaMemcpy failed: "
...
size_t CpuComplexSignal::size() const { return m_size; }

cuComplex *CpuComplexSignal::getData() const { return m_data; }

std::shared_ptr<IData> CpuComplexSignal::copy() const {
  if (!isValid()) {
    std::cerr << "CpuComplexSignal::copy failed: source data is invalid."
              << std::endl;
    return nullptr;
  }
