 ...
 */
inline bool isAllowedType(GpuSampleType value,
...
validateGpuInput(const std::shared_ptr<IData> &data,
                 std::initializer_list<GpuSampleType> allowedTypes,
                 size_t minSize, const std::string &context) {
  ...
  if (signal->size() < minSize) {
    return {false, context + ": input size is smaller than required minimum.",
            nullptr};
  }
 ...
 */
inline std::shared_ptr<IGpuSignalData>
createLike(const IGpuSignalData &prototype, size_t size) {
 ...
 */
inline ValidationResult
ensureHistoryLike(const IGpuSignalData &input, size_t historySize,
  ...
  const auto needsRecreate =
      [&](const std::shared_ptr<IGpuSignalData> &buffer) {
        return !buffer || buffer->sampleType() != input.sampleType() ||
  ...
  if (needsRecreate(history)) {
    ...
    if (!history || !history->isValid()) {
      return {false, "ensureHistoryLike: failed to create history buffer.",
              nullptr};
    }

    const auto clearErr = cudaMemset(history->deviceDataRaw(), 0,
                                     historySize * history->elementSizeBytes());
    if (clearErr != cudaSuccess) {
      return {false,
              "ensureHistoryLike: failed to zero-initialize history buffer.",
              nullptr};
    }
  } else if (!history->setLogicalSize(historySize)) {
    return {false, "ensureHistoryLike: failed to set history logical size.",
            nullptr};
  }
  ...
  if (needsRecreate(nextHistory)) {
    ...
    if (!nextHistory || !nextHistory->isValid()) {
      return {false, "ensureHistoryLike: failed to create next history buffer.",
              nullptr};
    }
  } else if (!nextHistory->setLogicalSize(historySize)) {
    return {false,
            "ensureHistoryLike: failed to set next history logical size.",
            nullptr};
  }
