...
Модули компилируются как разделяемые библиотеки (`.so`). `ModuleFactory` ищет путь к библиотеке по имени модуля в файловой системе в директории `Modules/<ModuleName>/` (файл вида `lib<ModuleName>-module.so`), затем загружает через `dlopen`/`dlsym` и вызывает фабричную функцию `createModule()`.
...
- **Динамическая загрузка:** модули загружаются через `dlopen` по путям из директории `Modules/<Name>/` — валидируйте пути при изменении логики ModuleFactory
