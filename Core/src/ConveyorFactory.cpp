...
bool ConveyorFactory::buildModule(
    ...
    if (!libPath.empty()) {
        metaData.jsonModuleFilePath = (std::filesystem::path(libPath).parent_path() / metaData.jsonModuleFilePath).string();
    }
    ...
    if (auto paramsIt = moduleObj.find("params"); paramsIt != moduleObj.end()) {
        ...
        for (const auto& kv : params) {
            ...
            module->fetchParam(paramName, convertParamValue(kv.value(), reader, paramName));
        }
    }
...
std::any ConveyorFactory::convertParamValue(
    const value& v,
    const ModuleMethaDataReader& reader,
    const std::string& paramName
) {
    // Строки оставляем как есть — fetchParam разрешит переменные ($VAR) и теги (@tag)
    if (v.is_string()) {
        return std::string(v.as_string().c_str());
    }
