...
bool ConveyorOrchestrator::run() {
  ...
  for (const auto &runtime : m_runtimes) {
    const std::string name =
        runtime.name.empty() ? std::string("<destroyed>") : runtime.name;
    INFO << "Conveyor '" << name << "' total time: " << runtime.elapsedSeconds
         << " s" << std::endl;
  }
...
bool ConveyorOrchestrator::buildConveyors() {
  const auto *rootObj = m_root.if_object();
  if (!rootObj) {
    ERROR << "ConveyorOrchestrator::buildConveyors: root is not object."
          << std::endl;
    return false;
  }
