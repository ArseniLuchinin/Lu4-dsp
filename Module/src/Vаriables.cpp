...
bool Variables::load(const std::string &filename) {
  ...
  try {
    ...
    std::cerr << "[Config Error] " << err.description() << " at "
              << err.source().begin << "\n";
    return false;
  }
}
...
bool Variables::processValue(const std::string &key, const toml::node &value) {
  // ENUM (string → int)
  if (value.is_string()) {
  ...
  // INT
  if (value.is_integer()) {
    data_[key] = static_cast<int64_t>(value.as_integer()->get());
    return true;
  }

  // FLOAT
  if (value.is_floating_point()) {
    data_[key] = static_cast<float>(value.as_floating_point()->get());
    return true;
  }
...
bool Variables::error(const std::string &key, const std::string &message,
                      const std::string &location) const {
  std::cerr << "[Config Error] key=\"" << key << "\" " << message;
