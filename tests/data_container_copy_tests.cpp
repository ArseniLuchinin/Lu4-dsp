...
TEST(DataContainerCopyTest, CpuComplexSignal_Copy_CreatesIndependentCopy)
{
    ...
    for (size_t i = 0; i < size; ++i) {
        hostData[i] = make_cuComplex(static_cast<float>(i), static_cast<float>(i) * 2.0f);
    }
...
TEST(DataContainerCopyTest, GpuComplexFloatSignal_Copy_CreatesIndependentGpuCopy)
{
    if (!isCudaAvailable()) {
        GTEST_SKIP() << "CUDA device is unavailable.";
    }
    ...
    for (size_t i = 0; i < size; ++i) {
        hostData[i] = make_cuComplex(static_cast<float>(i), static_cast<float>(i) * 3.0f);
    }

    auto original = std::make_shared<GpuComplexFloatSignal>(size);
    original->setDataFromHost(hostData.data(), size);

    auto copy = std::dynamic_pointer_cast<GpuComplexFloatSignal>(original->copy());
