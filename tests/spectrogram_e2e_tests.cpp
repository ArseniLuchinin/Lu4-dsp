...
TEST(SpectrogramE2E, MiniE2E_GeneratesPngFile)
{
    ...
    for (size_t i = 0; i < kInputSize; ++i) {
        const float phase = 2.0f * static_cast<float>(M_PI) * freq * static_cast<float>(i) / kSampleRate;
    ...
    const std::filesystem::path outPath = std::filesystem::temp_directory_path() / "spectrogram_e2e_test";
