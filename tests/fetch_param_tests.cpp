...
public:
  FetchParamProbeModule() : IModule({"FetchParamProbeModule", "", ""}) {}
  ...
  std::shared_ptr<IData> getData() override { return nullptr; }
...
TEST(FetchParamTests, TagTokenFetchesDataAndPassesSharedPtrToSetParam) {
  ...
  std::thread fetchThread(
      [&module, &tag]() { module.fetchParam("taps", std::string("@") + tag); });
  ...
  const auto fetchedData =
      std::any_cast<std::shared_ptr<IData>>(&module.m_lastValue);
...
TEST(FetchParamTests, VariableTokenStillResolvedThroughVariables) {
  FetchParamProbeModule module;
  const auto variablesPath = std::filesystem::temp_directory_path() /
                             "fetch_param_tests_variables.toml";
